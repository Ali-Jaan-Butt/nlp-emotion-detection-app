...
class myapp(MDApp):
    ...
    def prediction(self):
        ...
        if os.path.exists('model/nlp_model.h5')==True:
            ...
            txt = self.root.ids.sentence_input.text
            print(txt)
            txt = [txt]
            ...
            self.pred_label = Label(text='', color=(1, 1, 1, 1))
            self.pred_label.text = "Prediction is: "+prediction_res
            popup = Popup(title='Prediction', content=self.pred_label, size_hint=(None, None), size=(400, 200))
